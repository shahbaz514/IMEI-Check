...
<?php
$sqlServices = mysqli_query($db, "SELECT * FROM services WHERE category = '".$rowCat['id']."'");
                ...
                if (isset($_POST['edit']))
                {
                                    ...
                                    <div class="row" style="margin-top: 20px;">
                                        <div class="col-sm-12">
                                            <center>
                                                ...
                                                <h4 class="card-title">
                                                   You have insufficient balance to check Bulk IMEI.<br> Please Recharge your Wallet First!
                                                </h4>
                                            </center>
                                        </div>
                                    </div>
                    ...
                    {
                    foreach ($imeino_ar_new as $value) {
                        if ($imeino == null){
                            $err = 1;
                            ?>
                            <script>
                        ...
                        else if ($duplicate == true)
                        {
                            ...
                            $ch = curl_init($sqlGETSERVISE['link'] . $value);
                            ...
                            if ($httpcode != 200) {
                                ...
                                if ($sqlGetServiceRow) {
                                    ...
                                    $insertOrder = mysqli_query($db, "INSERT INTO `orders`(`orderid`,`status`, `service`, `imei`, `result`, `notes`, `credits`, `username`) VALUES ('$orderId', 'Success', '$services', '$value', '$result', '', '$cost', '" . $_SESSION['username'] . "')");
                                    if ($sendemail == true)
                                    {
                                        $msg = $result;
