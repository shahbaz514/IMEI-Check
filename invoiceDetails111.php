...
<div class="container">
    <div class="card">
        ...
        <center>
            ...
            ?>
            <?php
            if ($orderRow['payment_method'] == "USDT")
            {
            ?>
            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#exampleModalUSDT">
                View/Upload Payment Proof
            </button>
            <?php
            }
            ...
            <!-- Modal USDT -->
            <div class="modal fade" id="exampleModalUSDT" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel">View / Upload Payment Proof</h5>
                        ...
                        <div class="modal-body">
                            ...
                            if ($orderRow['payment_method'] == "USDT")
                            {
                                ...
                                <form action="" method="post" enctype="multipart/form-data">
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            <input type="text" name="transID" placeholder="Enter Your Transcation ID" class="form-control" required>
                                            <center>
                                                <input type="submit" name="upload" value="Upload" class="btn btn-danger">
                                            </center>
                                        </div>
                                        <div class="col-sm-12">
                                            ...
                                            if ($orderRow['prof'] != "")
                                            {
                                                ...
                                                <h5><?php echo $orderRow['transID']; ?></h5>
                                                <?php
                                            }
                                            ?>
                                        </div>
                                ...
                                if (isset($_POST['upload']))
                                {
                                    ...
                                    $sqlU = mysqli_query($db, "UPDATE `credits` SET prof = '$newfilename', transID = '$transID' WHERE invoice_no = '".$_GET['invoice']."'");
                                    if ($sqlU)
                                    {
                                        $move = move_uploaded_file($_FILES["prof"]["tmp_name"], "img/" . $newfilename);
                                        if ($move)
                                        {
                                            $total = $userRow['wallet'] + $orderRow['total_amount'];
                                            $upWallet = mysqli_query($db, "UPDATE users SET wallet = '$total' WHERE username = '".$userRow['username']."'");
                                            if ($upWallet)
                                            {
                                            $status = mysqli_query($db, "UPDATE credits SET status = 'Paid' WHERE invoice_no = '".$_GET['invoice']."'");
                                                
                                            }
                                            if ($status){
                                               header('Location: allTopUp.php');     
                                                }
                                        }
            ...
            <?php
            if ($_SESSION['role'] == 'Admin' && $orderRow['status'] != 'Paid' && ($orderRow['payment_method'] == 'Direct Transfer'))
            {
                ?>
